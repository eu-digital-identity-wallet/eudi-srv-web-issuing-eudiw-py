# coding: latin-1
###############################################################################
# Copyright (c) 2023 European Commission
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
###############################################################################
import requests 
import main_pytest


ENDPOINT = main_pytest.ENDPOINT_route_mdl

#   Route mdl

#                   Tests WORKING

#   /mdl
def test_pid_getmdl_statusCode_200():
    """
        -> status_code == 200 -> tudo bem
        -> status_code != 200 -> esta alguma coisa mal
    """
    response= requests.get(ENDPOINT + "?version=0.2&country=PT&certificate=&returnURL=https://127.0.0.1:4430/pid/returnpidtest&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t", verify=False)
    assert response.status_code == 200


#   /mdl empty country atribute
def test_pid_getmdl_statusCode_200_emptyCountry():
    """
        -> status_code == 200 -> tudo bem
        -> status_code != 200 -> esta alguma coisa mal
    """
    response= requests.get(ENDPOINT + "?version=0.2&country=&certificate=&returnURL=https://127.0.0.1:4430/pid/returnpidtest&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t", verify=False)
    assert response.status_code == 200


#                   Test of ERRORS

#   -> Error 12 - missing some mandatory fields - version
#   -> response.text = Error 12: Query with no version.
def test_pid_get_error_12():

    url = ENDPOINT + "?country=PT&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&certificate=&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t"
    response= requests.get(url, verify=False)

    assert response.text == "Error 12: Query with no version.\n"
    

#   -> Error 101 - missing some mandatory fields - country
#   -> response.text = Error 101: Missing mandatory pid/getpid fields.
def test_pid_getmdl_error_101_R():

    url = ENDPOINT + "?version=0.2&certificate=&returnURL=https://127.0.0.1:4430/pid/returnpidtest&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t"
    response= requests.get(url, verify=False)

    assert response.text == "Error 101: Missing mandatory pid/getpid fields."


#   -> Error 11 - missing some mandatory fields - returnURL
#   -> response.text = Error 11: Query with no returnURL.
def test_pid_getmdl_error_11():

    url = ENDPOINT + "?version=0.2&country=PT&certificate=&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t"
    response= requests.get(url, verify=False)

    assert response.text == "Error 11: Query with no returnURL.\n"
    

#   -> Error 101 - missing some mandatory fields - certificate
#   -> response.text = Error 101: Missing mandatory pid/getpid fields.
def test_pid_getmdl_error_101_C():

    url = ENDPOINT + "?version=0.2&country=PT&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t" 
    response= requests.get(url, verify=False)

    assert response.text == "Error 101: Missing mandatory pid/getpid fields."


#   -> Error 13 - wrong version or is not supported.
#   -> response.text = Error 13: Version is not supported.
def test_pid_getmdl_error_13():

    url = ENDPOINT + "?version=0.212&country=PT&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&certificate="
    response= requests.get(url, verify=False)

    assert response.text == "Error 13: Version is not supported.\n"
    

#   -> Error 102 - wrong country or code or country is not supported.
#   -> response.text = Error 102: Country is not supported.
def test_pid_getmdl_error_102():

    url = ENDPOINT + "?version=0.2&country=err&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&certificate=&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t"
    response= requests.get(url, verify=False)

    assert response.text == "Error 102: Country is not supported."


#   -> Error 103 - wrong certificate or certificate not correctly encoded.
#   -> response.text = Error 103: Certificate not correctly encoded - Incorrect padding
def test_pid_getmdl_error_103():

    url = ENDPOINT + "?version=0.2&country=PT&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&certificate=LS0tLS1CRUdJTiNlZFdmlRQllwWnNYYURvQmRhQ1FJZ00rZUFBUE5zNXErNnZ0SVR1R1pTUkdoN1U3U1VMdWNqWGZJNmU0N2IKSjI4PQotLS0tLUVORCBDRVJUSUZJQ0FURS0tLS0t&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t"
    response= requests.get(url, verify=False)

    assert response.text == "Error 103: Certificate not correctly encoded - Incorrect padding"


#   -> Error 14 - wrong url.
#   -> response.text = Error 14: URL not well formed.
def test_pid_getmdl_error_14():

    url = ENDPOINT + "?version=0.2&country=PT&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&certificate="
    response= requests.get(url, verify=False)

    assert response.text == "Error 14: URL not well formed.\n"


#------------------------- // ------------------------ // --------------------------------
#                                    VERSION 0.3

#   ->	/pid/getpidtest result validated: True -> empty country
def test_pid_getmdl_empty_country():
    params = "?version=0.3&country=&returnURL=" + main_pytest.ENDPOINT + "/pid/returnpidtest&certificate=&device_publickey=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZrd0V3WUhLb1pJemowQ0FRWUlLb1pJemowREFRY0RRZ0FFdUZ3R003VW4wMkc1RjFrSjVveVlNd3JOeVRhTAo0b0F6MXhmWm9xcnNSTVZXMERWV3RRNVdSR0hPb3hSSzJGd1hndm9GaENQdDFHTGd3eURKYXU0cW53PT0KLS0tLS1FTkQgUFVCTElDIEtFWS0tLS0t"

    response= requests.get(ENDPOINT + params, verify=False)

    assert response.text == "/pid/getpidtest result validated: True\n"



#    ->	/pid/getpidtest no device_publickey error 15
def test_pid_getmdl_error_15():
    url = ENDPOINT + "?version=0.3&country=PT&certificate=&returnURL="+ main_pytest.ENDPOINT + "/pid/returnpidtest"
    response= requests.get(url, verify=False)
    
    assert response.text == "Error 15: Query with no device_publickey\n"