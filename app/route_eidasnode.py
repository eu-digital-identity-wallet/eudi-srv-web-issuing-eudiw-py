...
def getlightrequest_openid():
    ...
    if not b:
        return redirect_getpid_or_mdl(request.args.get('version'), request.args.get('returnURL'), 301, [])
...
def getlightresponse():
    ...
    if not b: # if error in getting the attributes
        ...
        return render_template("openid/eidas_fail.html", Info="Login Failed.  Retries left:" + str(session["tries"]), link=cfgcountries.supported_countries["CW"]["pid_url_oidc"])    
...
def eidasnodeR2():
    ...
    (b, e) = handle_response(user_id)
    if not b: # if error in getting the attributes
        return redirect_getpid_or_mdl(session['version'], session['returnURL'], 303, [('error_str', e)])
    ...
    if not r['error_code'] == 0:
        return redirect_getpid_or_mdl(session['version'], session['returnURL'], r['error_code'], [])
    ...
    if not r1['error_code'] == 0:
        return redirect_getpid_or_mdl(session['version'], session['returnURL'], r1['error_code'], [])
