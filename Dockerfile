...
CMD ["sh", "-c", "cp ./config_secrets/config_secrets.py /root/eudi-srv-web-issuing-eudiw-py/app/app_config/ && cp -r ./pid-issuer /etc/eudiw/pid-issuer && export REQUESTS_CA_BUNDLE=./config_secrets/cert.pem && /root/eudi-srv-web-issuing-eudiw-py/venv/bin/flask run"]
